...
type Props = {
  ...
  units: string[];
  defaultUnit: string;
  projects: string[]; // kept for API parity; not used now
  initialCode?: string;
  editing?: Supply | null;
};
...
export const SupplyForm = ({ open, onOpenChange, units, defaultUnit, initialCode, editing }: Props) => {
  ...
  useEffect(() => { if (open) reset(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [open, editing]);
      ...
      if (editing) {
        ...
        if (stock > 0 && created) {
          await supabase.from("transactions").insert({
            ...
            type: "in", quantity: stock, project: null,
          });
        }
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">Each supply has one unified stock count. Per-project usage is tracked from checkouts.</p>
          </div>
