  ...
  const submit = async () => {
    ...
    try {
      ...
      const { data: existing } = await supabase
        .from("supplies").select("*").eq("user_id", user.id).eq("code", code.trim()).maybeSingle();
      if (existing) {
        const newStock = existing.stock + qty;
        const { error } = await supabase.from("supplies").update({ stock: newStock }).eq("id", existing.id);
        if (error) throw error;
        await supabase.from("transactions").insert({ user_id: user.id, supply_id: existing.id, type: "in", quantity: qty });
        ...
        const { data: created, error } = await supabase.from("supplies")
          .insert({ user_id: user.id, name: name.trim(), code: code.trim(), unit, stock: qty })
          .select().single();
        if (error) throw error;
