...
export const ScanStockInDialog = ({ open, onOpenChange, units, defaultUnit }: Props) => {
  ...
  const handleDetected = async (decoded: string) => {
    if (!user) return;
    setCode(decoded);
    const { data } = await supabase.from("supplies").select("id,name,unit").eq("user_id", user.id).eq("code", decoded).maybeSingle();
  ...
  const submit = async () => {
    ...
    try {
      if (existingId) {
        ...
        await supabase.from("transactions").insert({ user_id: user.id, supply_id: existingId, type: "in", quantity: qty });
        ...
        const { data: created, error } = await supabase.from("supplies")
          .insert({ user_id: user.id, name: name.trim(), code, unit, stock: qty }).select().single();
        if (error) throw error;
        await supabase.from("transactions").insert({ user_id: user.id, supply_id: created!.id, type: "in", quantity: qty });
